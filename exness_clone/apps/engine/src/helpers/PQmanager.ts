import { activeUsers, buyPQS, completedBuyOrders, completedLeverageBuyOrders, completedLeverageSellOrders, completedSellOrders, leverageBuyPQS, leverageSellPQS, livePrices, sellPQS } from "../variables/index.js";
...
export async function manageBuyPQS(data:any){
    ...
    while(1){
        ...
        if(topMostBuyOrder.stoploss>=sellPrice){
            ...
            const owner=topMostBuyOrder.owner
            if(!activeUsers[owner]){
                console.log('user does not exist in activeUsers');
                continue;
            } 
            const activeBuyOrders=activeUsers[owner].activeBuyOrders
            ...
            console.log('reserved amt of user changed from  ',reseredAmt,' to ',(reseredAmt+soldAmt));
...
export async function manageSellPQS(data:any){
    ...
    while(1){
        ...
        if(bottomMostSellOrder.stopPrice<=buyPrice){
            ...
            activeUsers[bottomMostSellOrder.owner].bal.usd.reserved=reserved+sellAmt-buyAmt
            console.log(`reserved amt of user : ${activeUsers[bottomMostSellOrder.owner]?.userData?.username} changed from ${reserved} to ${reserved+sellAmt-buyAmt}`);
...
export async function manageLeverageSellPQS(data:any){
    ...
    // console.log('data : ',data);
    const {symbol,buyPrice}=data;
    const pq=leverageSellPQS[symbol]
    if(!pq){
        // console.log('Priority queue has not been initialized for symbol : ',symbol);
        return;
    } else if(pq.size()==0){
    ...
    while(1){
        ...
        if(bottomMostLeverageSellOrder.stopPrice<=buyPrice){
            ...
            console.log(`reserved amt of user : ${activeUsers[bottomMostLeverageSellOrder.owner]?.userData?.username} changed from ${reserved} to ${reserved+netUserProfit}`);
